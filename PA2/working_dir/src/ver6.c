...
typedef struct Rtype{int rd, rs, rt, shamt, funct;}rtype;
typedef struct Itype{int rs, rt, short immediate;}itype;
typedef struct Jtype{unsigned int address;}jtype;

int bit_slice(int data, int left, int right);
void Rcommandread(int rs, int rt, int rd, int shamt, int funct);
void Icommandread(int rs, int rt, short immediate, int opcode);
void Jcommandread(unsigned int address, int opcode);

void Rcommand(int rs, int rt, int rd, int shamt, int funct);
void Icommand(int rs, int rt, short immediate, int opcode);
...
void Rcommand(int rs, int rt, int rd, int shamt, int funct) {
    if (funct == 2) {
        registers[rd] = (unsigned int)registers[rt] >> (unsigned int)shamt;
        //printf("srl $%d, $%d, %d", rd, rt, shamt);
    } else if (funct == 3) {
...
void Icommand(int rs, int rt, short immediate, int opcode){
    ...
    if (opcode == 8) {
        ...
        registers[rt] = (unsigned int)registers[rs] + (unsigned int)immediate;
        //printf("addiu $%d, $%d, %d", rt, rs, immediate);
    } else if (opcode == 12) {
        registers[rt] = registers[rs] & immediate;
        //printf("andi $%d, $%d, %d", rt, rs, immediate);
    ...
    //    printf("sb $%d, %d($%d)", rt, immediate, rs);
        ...
        registers[rt] = (unsigned int)registers[rt] <(unsigned int)immediate;
        //printf("sltiu $%d, $%d, %d", rt, rs, immediate);
    //} else if (opcode == 41) {
    //    printf("sh $%d, %d($%d)", rt, immediate, rs);
    //} else if (opcode == 43) {
