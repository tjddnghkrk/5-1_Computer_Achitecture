...
int main() {
    while (1) {
	 ...
	 for(int k = 0; k<32; k++)
		...
		opcode = bit_slice(instruction[pc], 26, 31);
		//printf("%d", opcode);

                if (opcode == 0) { //r
                    rtype rnum;
                    rnum.rs = bit_slice(instruction[pc], 21, 26);
                    rnum.rt = bit_slice(instruction[pc], 16, 21);
                    rnum.rd = bit_slice(instruction[pc], 11, 16);
                    rnum.shamt = bit_slice(instruction[pc], 6, 11);
                    rnum.funct = bit_slice(instruction[pc], 0, 6);
                    Rcommand(rnum.rs, rnum.rt, rnum.rd, rnum.shamt, rnum.funct);

                } else if (opcode > 3) {
		//	printf("hi");
                    itype inum;
                    inum.rs = bit_slice(instruction[pc], 21, 26);
                    inum.rt = bit_slice(instruction[pc], 16, 21);
                    inum.immediate = bit_slice(instruction[pc], 0, 16);
                    Icommand(inum.rs, inum.rt, inum.immediate, opcode);
                }
		else if(opcode == 2 || opcode == 3){
                    jtype jnum;
                    jnum.address = bit_slice(instruction[pc], 0, 26);
