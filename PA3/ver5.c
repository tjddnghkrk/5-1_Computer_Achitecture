...
void Icommand(uch rs, uch rt, short immediate, uch opcode){
    ...
    */} else if (opcode == 32) {
	unsigned char * lb;
	unsigned char temp;
	lb = instdata + (registers[rs]&0xFFF) + immediate;
	temp = *lb;
	registers[rt] = (int)temp;
	printf("%0x", registers[rt]);
//      printf("lb $%d, %d($%d)", rt, immediate, rs);
     } else if (opcode == 36) {
	unsigned char * lbu;
        unsigned char temp;
        lbu = instdata + (registers[rs]&0xFFF) + immediate;
        temp = *lbu;
...
//        printf("lbu $%d, %d($%d)", rt, immediate, rs);
	...
	short temp;
	//1000
	lh = instdata + (registers[rs]&0xFFF) + immediate;
	temp = *(short*)lh;
...
//        printf("lh $%d, %d($%d)", rt, immediate, rs);
	...
	unsigned char * lhu;
        short temp;
        //1000
        lhu = instdata + (registers[rs]&0xFFF) + immediate;
        temp = *(short*)lhu;
...
//        printf("lhu $%d, %d($%d)", rt, immediate, rs);
	...
	int temp;
	//1000
	lw = instdata + (registers[rs]&0xFFF) + (immediate); 
	printf("%0x\n", (registers[rs]&0xFFF) + immediate);
	temp = *(int *)lw;
	...
	unsigned char * sb = instdata + (registers[rs]&0xFFF) + immediate;
	*sb = temp;
    //    printf("sb $%d, %d($%d)", rt, immediate, rs);
    } else if (opcode == 10) {
        registers[rt] = registers[rs] <immediate;
	...
	unsigned char * sh = instdata + (registers[rs]&0xFFF) + immediate;
	*(short *)sh = temp;
    //    printf("sh $%d, %d($%d)", rt, immediate, rs);
    } else if (opcode == 43) {
	int temp = htonl(registers[rt]);
	unsigned char * sw = instdata + (registers[rs]&0xFFF) + immediate;
	*(int *)sw = temp;
    //    printf("sw $%d, %d($%d)", rt, immediate, rs);
    } else if (opcode == 14) {
        registers[rt] = (registers[rs] ^ (immediate & 0xFFFF));
...
int memory_range(short immediate)
{
	if(0x10000000 <= immediate && immediate <= 10001000)
		return 0;
	else
		return immediate;
}
